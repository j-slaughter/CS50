...
bool check(char *word)
{
    // Set node pointer to root of trie
    node *ptr = root;
    // Loop through each character in the word
    for (int i = 0, n = strlen(word); i < n; i++)
    {
        // Find the alphabet index number of the character
        int index = tolower(word[i]) - 'a';
        // Check if letter in trie
    ...
    // Check if it is a word after looping through all letters
    if (ptr->is_word == true)
    {
        return true;
    }
